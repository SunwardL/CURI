...
class Store:
    ...
    def summary(self, days: int = 0) -> dict[str, Any]:
        ...
        project_counts = Counter((row.get("project") or "unknown") for row in turns)
        ...
        turn_view = [{"ts": row.get("ts"), "project": row.get("project") or "unknown",
                      "model": row.get("reported_model") or row.get("requested_model") or "unknown",
                      "input_tokens": row.get("input_tokens") or 0, "cached_tokens": row.get("cached_tokens") or 0,
                      "output_tokens": row.get("output_tokens") or 0, "error": row.get("error_class")}
                     for row in turns[-500:]]
        return {"meta": {"generated_at": now_iso(), "last_scan": self.last_scan, "files": file_rows},
                ...
                "projects": [{"project": name, "turns": count} for name, count in project_counts.most_common()],
                "turns": turn_view,
...
function render(d){const t=d.today||{},r=d.relay||{},allTurns=d.turns||{};$('updated').textContent=`scanned ${d.meta.last_scan||'—'}`;$('cards').innerHTML=card('Today · tokens',n((t.input_tokens||0)+(t.output_tokens||0)),`${n(t.input_tokens)} in · ${n(t.output_tokens)} out`)+card('Today · turns',n(t.turns),`${n(t.model_calls)} identifiable model calls`)+card('Relay · success',r.total?`${Math.round(r.success*100/r.total)}%`:'—',`${n(r.total)} events · ${n(r.retries)} retries`,r.total&&r.success<r.total?'bad':'ok')+card('Capacity errors',n(t.capacity_errors),`${n(t.errors)} total local errors`,t.capacity_errors?'bad':'');
if(!$('modelFilter').dataset.ready){(d.models||[]).forEach(x=>$('modelFilter').insertAdjacentHTML('beforeend',`<option value="${esc(x.model)}">${esc(x.model)}</option>`));(d.projects||[]).forEach(x=>$('projectFilter').insertAdjacentHTML('beforeend',`<option value="${esc(x.project)}">${esc(x.project)}</option>`));$('modelFilter').dataset.ready='1'}const chosenModel=$('modelFilter').value,chosenProject=$('projectFilter').value,filtered=(Array.isArray(allTurns)?allTurns:[]).filter(x=>(!chosenModel||x.model===chosenModel)&&(!chosenProject||x.project===chosenProject));const daily=filtered.reduce((a,x)=>{const k=(x.ts||'').slice(0,10);const v=a[k]||(a[k]={date:k,input_tokens:0,output_tokens:0});v.input_tokens+=x.input_tokens||0;v.output_tokens+=x.output_tokens||0;return a},{});const days=Object.values(daily).slice(-14),max=Math.max(1,...days.map(x=>x.input_tokens+x.output_tokens));$('chart').innerHTML=days.length?days.map(x=>`<div class="bar" style="height:${Math.max(8,(x.input_tokens+x.output_tokens)*100/max)}%"><span>${esc(x.date.slice(5))}</span></div>`).join(''):'<div class="small">No session data for this filter.</div>';
...
const tools=d.tools.by_category||{};$('tools').innerHTML=Object.entries(tools).map(([k,v])=>`<div class="row"><span>${esc(k)}</span><strong>${n(v)}</strong></div>`).join('')||'<div class="small">No tool calls observed.</div>';const visibleModels=filtered.reduce((a,x)=>(a[x.model]=(a[x.model]||0)+1,a),{});$('models').innerHTML=Object.entries(visibleModels).map(([k,v])=>`<div class="row"><span>${esc(k)}</span><span class="tag">${n(v)} turns</span></div>`).join('')||'<div class="small">No model calls for this filter.</div>';
